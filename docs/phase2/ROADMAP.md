...
- [ ] **(1) Containerized Core + hosted-service bootstrap** (ENG-0010): single Linux container image (app + SSH client; **no `qemu`/`qm`** baked in). Bind **`0.0.0.0:{configurable port}`** (env-driven), dropping the `127.0.0.1:{random}` loopback + `FindFreePort()`. **Core-terminated HTTPS** via Kestrel with a self-signed (first-run, warn) or operator-provided cert; reverse-proxy is a **documented future seam, not required for release 1**. KEK + TLS cert/key **injected at runtime**, never baked in. SQLite on the mounted `VMENTORY_DB` volume. Re-scope `Updater.cs` from GitHub-exe to image tags. *This replaces the desktop bootstrap in `Program.cs` first.*
- [ ] **(2) Login + RBAC framework** (ENG-0008): minimal admin login replaces the single session token (in this same deployment slice). **Fixed roles** (Admin / VM-operator / Backup-operator / Viewer) over a **pillar×verb catalog** reusing `ProviderCapability` + a small console-permission set; **local accounts** now (hashed, KEK-wrapped), OIDC seam later; a **single audited authz chokepoint** in Web/API **and** the operations engine. **This framework must exist before any write verb is exposed** — roles before the first write verbs.
