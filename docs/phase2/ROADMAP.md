...
- [x] Persistence layer (SQLite + EF Core): host registry + inventory snapshots; diff fed from persisted snapshots; always-on in real mode, `--mock` ephemeral; `VMENTORY_DB` env override; **no secrets persisted** (await `ISecretStore`); `/api/quit` no longer purges. **Single-operator schema — no `tenant_id`** (ENG-0006). *Pending:* snapshot retention/cadence policy (open Q), Postgres option.

**Re-baselined foundation slices (approved 2026-06-16; this is the order they ship in):**
- [x] **(1) Containerized Core + hosted-service bootstrap** (ENG-0010) — **built & verified on `dev` 2026-06-17.** `Program.cs` rewritten loopback-desktop → hosted service: binds **`VMENTORY_HTTP_ADDR` (default `0.0.0.0`) : `VMENTORY_HTTP_PORT`** via `ConfigureKestrel`, dropping `127.0.0.1:{random}` + `FindFreePort()` (and `OpenBrowser`/R-key; console Q-loop now TTY-gated on `!Console.IsInputRedirected`; WinRM ensure `IsWindows()`-guarded). **Core-terminated HTTPS** via `listen.UseHttps` with new `TlsSetup.cs` (cert precedence **operator PFX → operator PEM → self-signed**, real-mode cached / mock ephemeral, nothing baked in); **`VMENTORY_HTTP_ONLY=1`** disables TLS for reverse-proxy/dev (a documented future seam, not required for release 1). Single multi-stage Linux `Dockerfile` + `.dockerignore` (app + `openssh-client`; **no `qemu`/`qm`**; **non-root uid 10001**; DB + cached cert on the **`/data` volume**; `EXPOSE 8443`); csproj manifest Windows-only-conditioned. **Verified:** build 0/0; mock-mode DLL run bound `0.0.0.0:8444` self-signed HTTPS (SPA 200, `/api/state` 401 without token / 200 with), `VMENTORY_HTTP_ONLY=1` plain HTTP 200 + 401 gate. *Carry-overs (NOT done):* the **session token persists as interim auth** (login + RBAC retires it in slice (2)); the **`/api/quit` desktop route** is left in place (graceful) pending retirement; **`Updater.cs` re-scope** GitHub-exe → image tags is still pending; the **container image is authored but unbuilt** (no Docker on this host).
- [ ] **(2) Login + RBAC framework** (ENG-0008) ← **NEXT**: minimal admin login **replaces the interim session token** that slice (1) carried forward. **Fixed roles** (Admin / VM-operator / Backup-operator / Viewer) over a **pillar×verb catalog** reusing `ProviderCapability` + a small console-permission set; **local accounts** now (hashed, KEK-wrapped), OIDC seam later; a **single audited authz chokepoint** in Web/API **and** the operations engine. **This framework must exist before any write verb is exposed** — roles before the first write verbs.
- [ ] **(3) `ISecretStore`** + app-native envelope encryption (AES-256-GCM, runtime-injected KEK) (ENG-0002), so provider creds/tokens (PVE API token, SSH key) and the local-account hashes persist encrypted.
- [ ] **(4) Proxmox API provider — read / planted Observe**: `ProxmoxProvider` PVE REST client (scoped API token); Proxmox hosts/VMs appear in the dashboard alongside Hyper-V. *(Read-only; matches what is broken out into 2.1 below.)*
