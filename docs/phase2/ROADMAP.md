...
- [~] Define `IVirtualizationProvider` + **capability model with management verbs gated per provider** (ENG-0007); generalize `Models.cs` into platform-neutral domain. **The capability model must allow management verbs (start/stop/reconfigure) on the Hyper-V provider — not source-only** — so 2.2 light HV management lands without reworking the abstraction (see [ARCHITECTURE.md → Capability model](ARCHITECTURE.md#capability-model--management-verbs-are-capability-gated-per-provider-eng-0007)). **Slice 2 done:** abstraction + full `ProviderCapability` flags in `VMentory.Core`; `Host.Platform` discriminator added; `HyperVProvider` (in `VMentory.Web`) advertises `Inventory|LiveStats` and the live HV inventory reads (quick-connect, scan) flow through the seam. *Pending:* lifecycle/migration **verb methods** + their HV advertisement (2.2), `ProxmoxProvider` (2.1).
...
- [~] Persistence layer (SQLite + EF Core): host registry + inventory snapshots. Migrate diff logic onto snapshots. **Single-operator schema — no `tenant_id`** (ENG-0006). **Slice 3 done:** `VMentory.Core/Persistence` (EF Core SQLite, `Initial` migration); host registry + inventory snapshots persist; diff fed from persisted snapshots; always-on in real mode, `--mock` ephemeral; `VMENTORY_DB` env override; **no secrets persisted** (await `ISecretStore`); `/api/quit` no longer purges. *Pending:* snapshot retention/cadence policy (open Q), formal `HostRegistration`/`HostInfo` split, Postgres option.
