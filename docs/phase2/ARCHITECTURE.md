...
> Status: **draft** · Owner: codebase side · Last decisions locked: 2026-06-16 (ENG-0001..0006)
>
...
## The product: four pillars on a shared foundation (ENG-0006)

VMentory is **not** "dashboard + management + one-way migration." It is a single-operator platform
that delivers **four pillars, each of standalone value**, sequenced
**Observe → Migrate → Deploy → Backup** — each shipping before the next, on one shared foundation
(the agent, provider model, secret store, PKI, persistence, and a general operations engine).

| Pillar | What it is | Status |
|---|---|---|
| 1. **Observe** | Resource/load dashboard: VM distribution, host utilization, capacity planning. **Not passive** — feeds *placement recommendations* consumed by Migrate and Deploy. | Phase-1 root; generalize |
| 2. **Migrate** | Wizard-driven **bidirectional HV ↔ PVE** (end state). **HV→PVE first** (proven by the [`migrate-vm`](../../migrate-vm/SKILL.md) skill). Handles Windows checkpoint resolution and live-vs-shutdown by **operator choice**. | HV→PVE = 2.3; bidir later |
| 3. **Deploy** | VM provisioning: ISO library + golden-image/template repository, creation wizard, target-hypervisor choice, **guest customization** (IP DHCP/static auto-config, post-deploy app install e.g. Atera with operator-supplied package). | After Migrate |
| 4. **Backup/Restore** | Backup + restore to **either** platform, reports, restore-testability, dedup, storage management, off-site replication. | After Deploy |

**PVE→HV is a distinct later effort**, not "the same wizard reversed" — it needs reverse driver
handling (strip virtio, inject Hyper-V drivers), qcow2→VHDX conversion, and Gen2/UEFI handling
(ENG-0006).
...
| 8 | Product scope | **Four pillars on one shared foundation**; sequenced **Observe → Migrate → Deploy → Backup**; **bidirectional migration end-state, HV→PVE first**; **backup buy-vs-build deferred**. | ENG-0006 |
...
- **Backup buy-vs-build** (ENG-0006) — orchestrate Proxmox Backup Server / Veeam vs build a native dedup/replication engine. **Explicitly deferred** — decided when pillars 1–3 are further along. A future ENG topic.
- **PVE→HV reverse migration** (ENG-0006) — the distinct later effort (reverse drivers, qcow2→VHDX, Gen2/UEFI). A future ENG topic.
